...
#[test]
fn test_limited_forward_split() -> Res<()> {
    set_verbose(true);

    let hay = Xell::from("1, 2, 3, 4, 5");
    let re = hay.to(r#"^split[", "][2]"#).err().unwrap();
    assert_eq!(re.sub().len()?, 3);

    assert_eq!(re.to("/[0]").read().value()?, "1");
    assert_eq!(re.to("/[1]").read().value()?, "2");
    assert_eq!(re.to("/[2]").read().value()?, "3, 4, 5");
    Ok(())
}

#[test]
fn test_limited_reverse_split() -> Res<()> {
    set_verbose(true);
    ...
    assert_eq!(re.to("/[0]").read().value()?, "1, 2, 3");
    assert_eq!(re.to("/[1]").read().value()?, "4");
    assert_eq!(re.to("/[2]").read().value()?, "5");
    Ok(())
}
